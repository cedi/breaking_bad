...
def RSA_PKCS1_v1_5_verify(public_key, sig, message):
    ...
    # ensure that signature starts with 0x00 0x01
    if sig_data[:2] != b'\x00\x01':
        return False
    
    # cut the header from signature
    sig_data = sig_data[2:]
    ...
    # 0x00 works as delimiter between padding and asn1
    if sig_data[0] != 0x00:
        return False

    # cut the delimiter from signature
    sig_data = sig_data[1:]
    ...
    # cut the asn1 from signature
    sig_data = sig_data[len(ASN_1_goop):]
 
    # hash the message
    hash_data = hashlib.sha1(message.encode('utf8')).digest()
    
    # if the hashed message and the signature matches
...
def handle_api():
    ...
    if not RSA_PKCS1_v1_5_verify(private_key.public_key(), sig, name):
        abort(403)
